<template>
    <div v-if="show" :class="['fixed h-full w-full flex items-center justify-center z-10', modalBackgroundClass]"
        @click="close">
        <div :class="['relative rounded-xl shadow-lg flex flex-col max-w-7xl p-1 w-[95%] md:w-[90%] h-[95%] md:h-[90%] overflow-hidden', modalShapeClass]"
            @click.stop>
            ...
            <svg class="absolute top-0 left-0 w-full h-full pointer-events-none" xmlns="http://www.w3.org/2000/svg">
                <rect width="100%" height="100%" fill="none" stroke-width="6" stroke-dasharray="400%" rx="12" ry="12"
                    ...
                    :class="['stroke-white/80 dark:stroke-zinc-700/80', modalRectangleClass]" />
            </svg>
        </div>
    </div>
</template>
...
const modalShapeClass = computed(() => exit.value ? 'animate-modal-shape-light-exit dark:animate-modal-shape-dark-exit' : props.transition ? 'bg-white/90 dark:bg-zinc-800/90' : 'animate-modal-shape-light dark:animate-modal-shape-dark');
